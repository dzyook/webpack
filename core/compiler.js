...
class Compiler {
  ...
  buildModule(entryName, modulePath) {
    ...
    const module = this.handleWebpackCompiler(entryName, modulePath);
    // 4. 返回对应module
    return module
  }
  ...
  handleWebpackCompiler(entryName, modulePath) {
      ...
      CallExpression: (nodePath) => {
        ...
        if (node.callee.name === 'require') {
          ...
          const absolutePath = tryExtensions(
            path.win32.join(moduleDirName, moduleName),
          ...
          const moduleId =
            './' + toUnixPath(path.win32.relative(this.rootPath, absolutePath));
    ...
    module.dependencies.forEach((dependency) => {
      const depModule = this.buildModule(entryName, dependency);
